...
function ETLKanjiSet.sampleValid(self, dst, path)
   -- Load the image into 'path' without changing anything.
   if not path then
      path = dst
      dst = torch.FloatTensor()
   end
   ...
   -- local out = input:crop(oW, oH, w1, h1)
   -- convert RGB to grayscale tensor
   out = input:toTensor('float','RGB','DHW', true)
   dst:resizeAs(out):copy(out)
   return dst
end
