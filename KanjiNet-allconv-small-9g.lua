...
model:add(cudnn.ReLU(true))
-- 5x5
model:add(cudnn.SpatialConvolution(256, 3036, 3,3, 1,1))
model:add(nn.SpatialAveragePooling(3,3, 1,1))
model:add(nn.View(3036))
...
------------- Dataset -------------
trainSet, validSet = ETLKanjiSet.load_9g(
...
TrainHelpers.trainForever(
   ...
   validSampler,
   validSet,
   "snapshots-kanjinet-9g/KanjiNet-allconv-small-nodropout-20150626",
   true, -- useCuda
   false, -- useTenCrops
   5 -- epochDropCount
)
